...
#### Bar plot - Categrical Variables vs SalePrice
...
for (i in 1:47){
  ...
  p[[i]]<- ggplot(data=train_df, aes_string (x= n[[i]], y="SalePrice")) +
...
lm_fit <- lm(Log_Sales_Price ~ .-GrLivArea,data = train_model %>%
               select_if(is.numeric) %>%
               select(-SalePrice))
